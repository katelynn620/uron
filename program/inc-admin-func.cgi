	...
	else
	{
		...
		foreach my $file (@files)
		{
			...
			if($inok && $outok)
			{
				...
				print OUT @data;
				close(OUT);
				push(@log,$file.l(' の復元に成功しました'));
			}
			else
			{
				close(IN) if $inok;
				push(@log,$file.l(' の復元に失敗しました',' 再度処理を行ってください'));
			}
		}
	}
}
...
sub delete_evt
{
	...
	print OUT @data;
	close(OUT);
	push(@log,$DATA_FILE.$FILE_EXT.l('内のイベントデータを削除しました'));
	}
	else
	{
	push(@log,$DATA_FILE.$FILE_EXT.l('内イベントデータ削除に失敗しました'));
	}
}
