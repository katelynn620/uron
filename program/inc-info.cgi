...
if(!$MOBILE)
{
	...
	$disp.=$TR."<td colspan=2 class=b>".l('今期売上').$TD.GetMoneyString($DT->{saletoday}).$TDB.l('前期売上')."<td colspan=2>\\".$DT->{saleyesterday}.$TRE;
	$disp.=$TR."<td colspan=2 class=b>".l('今期支払').$TD.GetMoneyString($DT->{paytoday}).$TDB.l('前期支払')."<td colspan=2>\\".$DT->{payyesterday}.$TRE;
	...
	$disp.=   $TDB.l('前期維持費')."<td colspan=2>\\".$DT->{costyesterday}.$TRE;
	$disp.=$TR."<td colspan=2 class=b>".l('今期売却税').$TD.GetMoneyString($DT->{taxtoday}).$TDB.l('前期売却税')."<td colspan=2>\\".($DT->{taxyesterday}+0).$TRE;
	...
	$disp.=$TR."<td colspan=2 class=b>".l('優勝回数').$TD.($DT->{rankingcount}+0)."回 ".GetTopCountImage($DT->{rankingcount}+0).$TRE;
	$disp.=$GUILD{$DT->{guild}} ? $TDB.l('ギルド会費')."<br><SMALL>(".l('決算時徴収').")</SMALL><td colspan=2>\\".int($DT->{saletoday}*$GUILD{$DT->{guild}}->[$GUILDIDX_feerate]/1000)."<SMALL>/".l('売上の').($GUILD{$DT->{guild}}->[$GUILDIDX_feerate]/10)."%</SMALL>" : $TD."　<td colspan=2>　";
	$disp.=$TRE.$TBE;
}
else
{
